  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const currentUser = JSON.parse(localStorage.getItem('currentUser') || '{}');
          if (payload.new.employee_id === currentUser.employee_id) {
            setTeacherProfile({
              name: payload.new.name || 'Teacher User',
              email: payload.new.email || 'teacher@example.com',
          ...
          <div className="p-3 sm:p-4 border-t border-slate-100">
            <div className="flex items-center space-x-2 sm:space-x-3 bg-gradient-to-r from-slate-50 to-blue-50 rounded-xl sm:rounded-2xl p-2 sm:p-3">
              <Avatar className="h-8 w-8 sm:h-10 sm:w-10 ring-2 ring-white shadow-md">
                <AvatarImage src={teacherProfile.profilePicture} alt={teacherProfile.name} />
                <AvatarFallback className="bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold text-xs sm:text-sm">
                  {teacherProfile.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()}
                </AvatarFallback>
              </Avatar>
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-semibold text-slate-900 truncate">{teacherProfile.name}</p>
                <p className="text-xs text-slate-500 truncate">{teacherProfile.employeeId}</p>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              ...
              <div className="flex items-center space-x-2 sm:space-x-3 bg-gradient-to-r from-slate-50 to-blue-50 rounded-xl sm:rounded-2xl p-1.5 sm:p-2 pr-2 sm:pr-4">
                ...
                <div className="hidden md:block">
                  <p className="text-xs sm:text-sm font-semibold text-slate-900">{teacherProfile.name}</p>
                  <p className="text-xs text-slate-500">{teacherProfile.employeeId}</p>
                </div>
              </div>
